...
var (	
	...
	Tjson = p.NewJSONBAccess(p.MakeColumnInfo(tableInfo, "tjson", p.NotPrimary, p.Nullable, 4),
			nil, func(dest interface{}, v *string) { dest.(*Thing).Tjson = v })
	// package private
	_ = time.Now
...
func (e *Thing) Setters() (list []p.ColumnAccessor) {
	...
	if e.Ttimestamp.Valid {
		list = append(list, Ttimestamp.Set(e.Ttimestamp.UNKOWN:timestamp without time zone))
	}
	if e.Tjson.Valid {
		list = append(list, Tjson.Set(e.Tjson.UNKOWN:jsonb))
	}	
	return
}
